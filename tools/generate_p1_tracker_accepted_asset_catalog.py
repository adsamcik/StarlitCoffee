#!/usr/bin/env python3
"""Generate the non-displayable registry for tracker-accepted P1 artwork.

Tracker acceptance proves that an illustration passed its image QA. It does
not provide reviewed, localized companion instruction or alt text, so this
generator deliberately produces *candidate* records rather than immediately
shippable InstructionAssetRecord instances. Runtime promotion remains gated in
the generated Kotlin by both an explicit reviewed localization record and the
existing recipe-level localization coverage authority.
"""
